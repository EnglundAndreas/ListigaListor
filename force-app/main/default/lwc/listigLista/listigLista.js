...
import soqlqueryparser from "@salesforce/resourceUrl/soqlqueryparser";
...
export default class ListigLista extends NavigationMixin(LightningElement) {
  ...
  connectedCallback() {
    ...
    if ((this.query || "").trim()) {
      loadScript(this, soqlqueryparser).then(() => {
        const parsedQuery = this.parseQuery(this.query);
  ...
  filterRecords(startingIndex) {
    if (this.data && this.data.length > 0) {
      ...
      this.totalPages = Math.ceil(
        this.totalNumberOfRows / Number(this.rowsPerPage)
      );
      var viewRecords = [];
